function PP_GP_subsets %(Data, in_index, out_index)
...
Setslog = [];
%=====================Define parameters=======================================
Data = 'number.mat';         %Data file
DATANAME='name.mat';      %variable names
in_index = [1:8];               %independent variables column no.
out_index = [9];                %dependent variable column no.

Setslog.evo_type = 2;  %set 1 for only Biobj evolution and 2 for first single obj followed by Biobj evolution
...
Setslog.max_depth = 3;              %max depth to which a tree grows
Setslog.max_roots = 3;              %max subtrees that a tree grows

subsets = 1; overlap = 5;           % previous 2 and 20 number of partitions of datafile and overlap b/w them
...
Setslog.generation1 = 10;           %generations for single obj evolution set Setslog.evo_type = 2
Setslog.generations = 50;          %max generations for evolution (initial 20)
Setslog.maxrank = 30;               %maxrank retained at KillInterval
Setslog.KillInterval = 10;          %Interval at which bad preys are eliminated
Setslog.Prey_popsize = 500;         %Initial popsize
Setslog.no_Prey_preferred = 300;    %Desired popsize
Setslog.no_new_Prey = 200;          %new prey introduced every KillInterval
Setslog.Predator_popsize = 30;     %Number of Predators
Setslog.no_x = 50;                  %lattice size (no of rows)
Setslog.no_y = 50;                  %lattice size (no of cols)
Setslog.tour_size = 5;              %tournament size for single objective GP
Setslog.ploton = 1;                 %set 0 for no plots or 1 for plots at every generation
Setslog.lambda = 0.5;               %parameter to evaluate complexity of tree = lambda*depth + (1-lambda)*nodes
Setslog.err_red_lim = 1e-3;         %any subtree contibuting less than this value for error reduction is eliminated
%================================DND==========================================
if Setslog.ploton
...
warning('off', 'all')
eval(['delete Setslog' num2str(Setslog.evo_type) '-' num2str(Setslog.max_roots) '-' num2str(Setslog.out_index) '_' Data])
warning('on', 'all')
%============================================
...
for i = 1:no_run
    ...
    Setslog.dataset(i).out = Setslog.DataSet(chosen_data_index,out_index);
end
%====================================
...
%====================================
eval(['save Setslog' num2str(Setslog.evo_type) '-' num2str(Setslog.max_roots) '-' num2str(Setslog.out_index) '_' Data ' Setslog'])
for i = 1:length(plst)
    rmpath([pwd plst{i}]);
end
clear all
