function PP_NNGA_subsets %(Data, in_index, out_index)
...
out_index = [10];                %dependent variable column no.
...
Setslog.generations = 100;  % 10 max generations for evolution
Setslog.nonodes = 5;       %maximum number of nodes
...
Setslog.KillInterval = 7;  %Interval at which bad preys are eliminated
Setslog.Prey_popsize = 500;        %500 Initial popsize
Setslog.no_Prey_preferred = 300;   %500 Desired popsize
Setslog.no_new_Prey = 200;          % 500 new prey introduced every KillInterval
Setslog.Predator_popsize = 50;     %Number of Predators
Setslog.no_x = 60;                  %lattice size (no of rows)
Setslog.no_y = 60;                  %lattice size (no of cols)
...
%====================================
eval(['save Setslog' num2str(Setslog.nonodes) '-' num2str(Setslog.out_index) '_' Data ' Setslog'])
for i = 1:length(plst)
    rmpath([pwd plst{i}]);
end
clear all
